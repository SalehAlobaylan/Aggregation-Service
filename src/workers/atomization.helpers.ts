...
const SHORT_CHAPTER_UNMERGEABLE_REASON =
  "Chapter below 4:30 and cannot merge without exceeding hard max.";
const MIN_CHAPTER_REVIEW_REASON =
  "Chapter is below the 4:30 minimum feed duration.";
const HARD_MAX_CHAPTER_REVIEW_REASON = "Chapter exceeds hard maximum duration.";
const PLANNER_FALLBACK_REASON =
  "Fallback single chapter; planner returned no usable chapters.";
const DURATION_BUCKET_MS = [5, 10, 15, 20, 30, 40].map(
...
function deriveStudioReviewCodes(
  chapter: AtomizationChapter,
  highConfThreshold: number,
): { primary: string | null; codes: string[] } {
  const codes: string[] = [];
  if (chapter.contains_sponsor_intro) codes.push("sponsor_intro");
  const reason = chapter.needs_review_reason ?? "";
  if (reason.includes("planner returned no usable chapters"))
    codes.push("planner_fallback");
  const mergedShort = chapter.merged_short_provenance === true;
  if (
    typeof chapter.confidence === "number" &&
    chapter.confidence < highConfThreshold
  )
    codes.push("low_confidence");
  if (mergedShort) codes.push("merged_short");
  if (reason.includes("cannot merge without exceeding hard max"))
    codes.push("short_unmergeable");
  if (reason.includes("below the 4:30 minimum feed duration"))
    codes.push("below_min");
  if (reason.includes("exceeds hard maximum duration"))
    codes.push("above_hard_max");
  if (codes.length === 0) return { primary: null, codes: [] };
  const primary =
    STUDIO_REVIEW_CODE_PRECEDENCE.find((c) => codes.includes(c)) ?? null;
  return { primary, codes };
}

export function annotateReviewCodes(
  chapters: AtomizationChapter[],
  highConfThreshold: number,
): AtomizationChapter[] {
  return chapters.map((chapter) => {
    const { primary, codes } = deriveStudioReviewCodes(
      chapter,
      highConfThreshold,
    );
    return {
      ...chapter,
      needs_review_code: primary,
      needs_review_codes: codes,
    };
  });
}

export function shouldAtomizeParent(
  durationSec?: number | null,
  minParentSec = DEFAULT_ATOMIZATION_MIN_PARENT_SEC,
): boolean {
...
/**
 * A policy's editorial preference may influence the request sent to the
 * planner, but it must never make legal full coverage impossible.  A parent
 * that is longer than the feed-unit maximum needs at least this many units.
 */
export function minimumCoverageChapterCount(
  durationSec: number,
  hardMaxSec: number,
): number {
  if (
    ...
    durationSec <= 0 ||
    !Number.isFinite(hardMaxSec) ||
    hardMaxSec <= 0
  ) {
    return 0;
  }
  return Math.ceil(durationSec / hardMaxSec);
}

export function planningChapterCount(input: AtomizationInputResponse): number {
  const preferred = Math.max(1, input.policy.max_chapters_per_parent || 1);
  const hardMaxSec = Math.max(1, input.policy.hard_max_chapter_minutes * 60);
  return Math.max(
    preferred,
    minimumCoverageChapterCount(input.item.duration_sec ?? 0, hardMaxSec),
  );
}
...
export function normalizeGeneratedChapters(
  ...
  const normalized = sorted.map((chapter, i) => {
    ...
    const nextStart = sorted[i + 1]
      ? byIndex.get(sorted[i + 1]!.start_index)?.start_sec
      : undefined;
    const explicitEnd =
      chapter.end_index != null
        ? endSecForWindow(chapter.end_index, windows, durationMs)
        : undefined;
    const endSec =
      explicitEnd ??
      nextStart ??
    ...
    const needsReview =
      ...
      (boundedEndMs - startMs > hardMaxMs
  ...
  const threshold =
    input.effective_policy?.high_confidence_threshold ??
...
export function mergeShortChapters(
  ...
  while (changed) {
    ...
    const index = merged.findIndex(
      (chapter) => chapter.end_ms - chapter.start_ms < minMs,
    );
    if (index < 0) break;
    ...
    if (neighbor == null) {
      merged[index] = {
        ...merged[index]!,
        needs_review_reason:
          merged[index]!.needs_review_reason ??
    ...
    const [first, second] =
      index < neighbor ? [index, neighbor] : [neighbor, index];
  ...
  return merged.map((chapter) => {
    ...
    if (duration < minMs) {
      return {
        ...chapter,
        needs_review_reason:
          chapter.needs_review_reason ?? MIN_CHAPTER_REVIEW_REASON,
      };
    }
    if (duration > hardMaxMs) {
      return {
        ...chapter,
        needs_review_reason:
          chapter.needs_review_reason ?? HARD_MAX_CHAPTER_REVIEW_REASON,
      };
    }
    return chapter;
  });
...
function chooseShortChapterNeighbor(
  chapters: AtomizationChapter[],
  index: number,
  hardMaxMs: number,
): number | null {
  const candidateIndexes = [index - 1, index + 1].filter(
    (i) => i >= 0 && i < chapters.length,
  );
  const candidates = candidateIndexes
    .map((i) => ({
      index: i,
      score: shortMergeScore(chapters, index, i, hardMaxMs),
    }))
    .filter(
...
function shortMergeScore(
  ...
  const sameContext =
    ...
    short.context_label === candidate.context_label
      ? 0
      : 1;
  const gapMs =
    candidateIndex < shortIndex
      ? Math.max(0, short.start_ms - candidate.end_ms)
      : Math.max(0, candidate.start_ms - short.end_ms);
  const bucketDelta = Math.min(
    ...DURATION_BUCKET_MS.map((bucket) => Math.abs(bucket - combinedMs)),
  );
  return sameContext * 1_000_000_000 + gapMs + bucketDelta / 100;
}

function mergeChapterPair(
  a: AtomizationChapter,
  b: AtomizationChapter,
): AtomizationChapter {
  ...
  const summary = [first.summary, second.summary]
    .map((value) => value?.trim())
    .filter(Boolean)
    .join(" ");
  const boundaryReasons = [
    first.boundary_reason,
    second.boundary_reason,
    "merged_short_chapter",
  ]
    .map((value) => value?.trim())
    .filter(
  ...
  const reviewReasons = [first.needs_review_reason, second.needs_review_reason]
    ...
    .filter(
      (value, index, list): value is string =>
        Boolean(value) &&
        value !== MIN_CHAPTER_REVIEW_REASON &&
        value !== SHORT_CHAPTER_UNMERGEABLE_REASON &&
  ...
  return {
    ...
    context_label:
      first.context_label === second.context_label
        ? first.context_label
        : (longer.context_label ??
          first.context_label ??
          second.context_label ??
          null),
    boundary_reason: boundaryReasons.join("; "),
    merged_short_provenance: true,
    standalone_score: Math.min(
      first.standalone_score ?? first.confidence ?? 0.72,
      second.standalone_score ?? second.confidence ?? 0.72,
    ),
    contains_sponsor_intro: Boolean(
      first.contains_sponsor_intro || second.contains_sponsor_intro,
    ),
    needs_review_reason:
...
export function endSecForWindow(
  index: number,
  windows: ChapterWindowPayload[],
  durationMs: number,
): number | undefined {
...
export function countReviewChapters(
  chapters: AtomizationChapter[],
  threshold: number,
): number {
  return chapters.filter(
    (ch) => (ch.confidence ?? 0) < threshold || !!ch.needs_review_reason,
  ).length;
}
...
export function compatibilityChapterChildren<
  T extends { delivery_mode: "legacy" | "shadow" | "durable_required" },
>(children: T[]): T[] {
